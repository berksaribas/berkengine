...
public class FollowerCamera extends Camera {
    ...
    public FollowerCamera(TransformComponent transformComponent, float eyeHeight) {
        super();
    ...
    public void followObject() {
        ...
        Matrix4f playerRealPosition = MatrixHelper.createTransformationMatrix(transformComponent.position,
        ...
        Vector3f rot = new Vector3f(rotation.x * rotation.angle, rotation.y * rotation.angle,
                rotation.z * rotation.angle);

        eye = new Vector3f((float) (pos.x - 0.025f * Math.sin(rot.y)), pos.y + 0.25f,
                (float) (pos.z - 0.025f * Math.cos(rot.y)));

        center = new Vector3f((float) ((eye.x - 1 * Math.cos(yAngle) * Math.sin(rot.y))),
                eye.y + (float) (1 * Math.sin(yAngle)), (float) ((eye.z - 1 * Math.cos(yAngle) * Math.cos(rot.y))));

        up = new Vector3f((float) (Math.sin(rot.y) * Math.sin(yAngle)), (float) Math.cos(yAngle),
                (float) (Math.cos(rot.y) * Math.sin(yAngle)));
    }

    public void handleFPSMovement(float delta) {

        transformComponent.rotation.y -= (float) MouseHandler.getDX() / (60000f * delta + 0.0000001f);
